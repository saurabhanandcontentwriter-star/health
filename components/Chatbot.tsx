...
const Chatbot: React.FC<ChatbotProps> = ({ doctors, labTests, onBookAppointment, onBookLabTest, setCurrentView, onStartVideoCall, newMessage, onNewMessageConsumed }) => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            if (attachedFile) {
                ...
                const prompt = trimmedInput || `Please analyze this document/image. If it's a health report, summarize the key findings.`;
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className="fixed bottom-24 right-6 w-full max-w-sm h-[70vh] max-h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col z-50 border border-gray-200 animate-slide-in">
                    ...
                    <div className="flex-1 p-4 overflow-y-auto bg-gray-50">
                            ...
                            {messages.map((msg, index) => (
                                <div key={index} className={`flex items-start gap-2.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    <div className={`flex flex-col gap-1 w-full max-w-[320px] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                                        <div 
                                            className={`relative flex flex-col p-0 rounded-2xl transition-all ${msg.role === 'user' ? 'bg-blue-500 rounded-br-none' : 'bg-gray-200 rounded-bl-none'}`}
                                        >
                                            <div 
                                            ...
                                            >
                                                ...
                                                <p className={`text-sm p-3 ${msg.role === 'user' ? 'text-white' : 'text-gray-800'}`} style={{ whiteSpace: 'pre-wrap' }}>
                                                    {msg.text}
                                                </p>
                                            </div>
                                            ...
                                            {msg.doctors && (
                                                <div className="space-y-2 p-2">
                                                    {msg.doctors.map(doc => {
                                                        ...
                                                        return (
                                                            <div key={doc.id} className="w-full text-left bg-white rounded-lg shadow-sm border border-gray-200 transition-all duration-300">
                                                                <button onClick={(e) => { e.stopPropagation(); handleToggleDoctorSlots(index, doc.id); }} className="w-full p-3 flex items-center space-x-3 hover:bg-teal-50">
                                                                ...
                                                                {isExpanded && (
                                                                    <div className="p-3 pt-3 mt-2 border-t border-gray-100 animate-fade-in-fast">
                                                                        <h4 className="text-sm font-semibold text-gray-700 mb-2">Available Slots:</h4>
                                                                        <div className="flex flex-wrap gap-2">
                                                                            {slots.length > 0 ? slots.map(slot => (
                                                                                <button key={slot} onClick={(e) => { e.stopPropagation(); handleSlotSelect(doc, slot); }} className="px-3 py-1.5 border border-teal-200 bg-teal-50 text-teal-700 rounded-md text-sm font-medium hover:bg-teal-100">
                                                                                    {slot}
                                                                                </button>
                                                                            )) : (
                     ...
                     <div className="p-2 text-center text-xs text-gray-500 border-t bg-gray-100">
                        AI assistant is not a medical professional. Always consult a qualified doctor for medical advice.
                    </div>
                    <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-200 flex items-center bg-white rounded-b-2xl relative">
                        ...
                        <div className="flex items-center gap-2">
                            {showAttachmentOptions && (
                                <div className="flex items-center gap-2 animate-fade-in-fast" >
                                     <button type="button" onClick={() => { setShowFaceScan(true); setShowAttachmentOptions(false); }} className="p-2 text-gray-500 hover:text-teal-600 rounded-full hover:bg-gray-100" aria-label="Scan face">
                                        <CameraIcon className="w-6 h-6" />
                                    </button>
                                    <button type="button" onClick={() => imageInputRef.current?.click()} className="p-2 text-gray-500 hover:text-teal-600 rounded-full hover:bg-gray-100" aria-label="Attach image">
                                        <ImageIcon className="w-6 h-6" />
                                    </button>
                                    <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2 text-gray-500 hover:text-teal-600 rounded-full hover:bg-gray-100" aria-label="Attach file">
                                        <PaperclipIcon className="w-6 h-6" />
                                    </button>
                                </div>
                            )}
                             <button type="button" onClick={() => setShowAttachmentOptions(!showAttachmentOptions)} className={`p-2 rounded-full transition-all duration-300 ${showAttachmentOptions ? 'bg-teal-100 text-teal-600 rotate-45' : 'text-gray-500 hover:text-teal-600'}`}>
                        ...
                        <div className="flex-1 mx-2 relative">
                            {selectedFile && (
                                <div className="absolute bottom-full left-0 w-full bg-gray-100 p-2 rounded-t-lg text-sm text-gray-700 flex justify-between items-center animate-fade-in-fast">
                            ...
                            <input
                                ...
                                className="w-full px-4 py-2 bg-gray-100 rounded-full border border-transparent focus:outline-none focus:ring-2 focus:ring-teal-500"
                                disabled={isLoading}
                                onKeyDown={(e) => {
